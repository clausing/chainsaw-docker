...
LABEL version="chainsaw 2.15.0 + Sigma r2026-04-01 Core++"
LABEL description="Run chainsaw in a docker container"

RUN apt update && \
    apt install wget unzip libterm-readline-perl-perl -y && \
    apt clean && \
    wget https://github.com/WithSecureLabs/chainsaw/releases/download/v2.15.0/chainsaw_all_platforms+rules.zip && \
    unzip chainsaw*.zip && \
